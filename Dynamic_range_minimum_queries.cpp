...
void buildSeg(vl &v, vl &seg, ll l, ll r, ll i){
	if(l==r){
		seg[i] = v[l];
	}
	else{
...
void updateQ(vl &seg, ll l, ll r, ll k, ll u, ll i){
	if(l==r){
		...
		ll mid = l+(r-l)/2;
		if(k<=mid) updateQ(seg,l,mid,k,u,2*i+1);
		else updateQ(seg,mid+1,r,k,u,2*i+2);
		seg[i] = min(seg[2*i+1],seg[2*i+2]);
	}
}
